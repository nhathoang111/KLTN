...
const AttendanceManagement = () => {
  ...
  const viewOnlyStudentId = useMemo(() => {
    if (isParent) return activeStudentId ? Number(activeStudentId) : null;
    if (isStudent) return user?.id ? Number(user.id) : null;
    return null;
  }, [activeStudentId, isParent, isStudent, user?.id]);
  ...
  const loadRoster = useCallback(async () => {
    if (isViewOnly) {
      if (!viewOnlyStudentId) {
        setItems([]);
        setError('Chưa có con được chọn để xem điểm danh.');
        return;
      }
      ...
      try {
        ...
        setError('');
        setMessage('');
        const targetDate = date || getTodayYmd();

        const res = await api.get('/attendance', {
          params: { studentId: Number(viewOnlyStudentId), date: targetDate },
        });

        const roster = (res.data?.items || res.data?.attendance || []).map((it) => ({
          studentId: it?.studentId ?? viewOnlyStudentId,
          fullName: it?.fullName || user?.fullName || '—',
  ...
  }, [date, selectedClassSectionId, isViewOnly, viewOnlyStudentId, user?.fullName, user?.email]);
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-collapse text-sm">
              ...
              <tbody className="text-slate-700">
                {items.length === 0 && !loadingRoster && (selectedClassSectionId || isViewOnly) && (
                  <tr>
                    <td colSpan={4} className="px-4 py-8 text-center text-slate-500">
                      Không có dữ liệu điểm danh cho con và ngày đã chọn.
                    </td>
                  </tr>
                )}
                {items.map((it) => (
